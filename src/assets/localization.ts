...
I like Todorant, it lets me free up my "operating memory" and allows me to think about solving problems, not about remembering them. As Nikita (the creator) correctly wrote — with constant and correct use, tasks that used to cast true biblical fear will stop appearing. And this will not happen because the complex tasks will stop coming. It will become clear to you (and to me, and everyone) that "the road will be overpowered by those walking", "the way to the thousand miles begins with one step".`,
    ...
    howto: {
      ...
      rules: [
        ...
        'If the current task is too high level, then break it down to a list of subtasks. As soon as you break it down, mark the task as completed.',
        ...
        'Feel free to join our multi-language <a target="_blank" href="https://t.me/todorant_feedback">group</a> on Telegram to share experience and tricks on how to be more productive! Your feedback about the service — suggestions, wishes and complaints — are highly welcome.',
        'Todorant does not have an option to assign specific time to a task — it is on purpose. Use the edit mode (the list order icon on top) on the <b>Planning</b> page to change the task order instead. If you want to assign a task to a specific week, just assign it to Monday and reassign it during the planning phase on Monday morning.',
...
Мне Тудурант нравится, и он позволяет освободить свою "операционную память", позволить себе мыслить над решением проблемы, а не над её запоминанием. Как правильно написал Никита (создатель) — при постоянном и правильном использовании перестанут появляться задачи, которые раньше внушали поистине библейский страх. И это будет происходит не из-за того, что перестают поступать сложные задачи. Тебе (и мне, и всем) станет понятно — "дорогу осилит идущий", "путь в тысячу ли начинается с одного шага".`,
    ...
    howto: {
      ...
      rules: [
        ...
        'Если текущая задача слишком высокоуровневая, то разбейте ее на список подзадач. Как только вы разбили задачу на подзадачи, завершите текущую задачу.',
        ...
        'Не стесняйтесь присоединяться к нашей <a target="_blank" href="https://t.me/todorant_feedback">группе</a> в Телеграме для обмена опытом о том, как становиться продуктивнее! Ваши отзывы о работе сервиса — предложения, пожелания и жалобы — крайне приветствуются.',
        'В Тудуранте нельзя создать задачу на определенное время — так и должно быть. Вместо этого, пользуйтесь режимом редактирования (иконка смены порядка сверху) на странице <b>Планирования</b> для изменения порядка задач. Если вы хотите создать задачу на определенную неделю — просто добавьте ее на понедельник и перераспределите во время планирования утром понедельника.',
      ...
      texts: {
        ...
        consequences: {
          ...
          points: [
            ...
            'Неприємні завдання (лягушки) завжди виконуються',
        ...
        reviews: [
          {
            name: 'Sergey Romanov',
            username: 'Gendoor',
            text: `Милі повідомлення у розділі підписки Тудуранту :)

Дякую за простий і прагнущий до ідеального сервіс. Виявилося, що він допомагає бути не тільки продуктивним, але й проактивним. Маленькі покращення кожен день впевнено підтягують за собою відстаючі ланки, а вільний від завдань розум постійно шукає, чим заповнити спорожнілий список завдань, щоб знов і знов отримувати задоволення від їх виконання і пов'язаних з цим змін — і це прямий шлях до покращення всього, що мене оточує.
...
Мені Тудурант подобається, і він дозволяє звільнити свою "операційну пам'ять", дозволити собі мислити над вирішенням проблеми, а не над її запам'ятовуванням. Як правильно писав Микита (творець) — при при постійному і правильному використанні перестануть з'являтися завдання, які раніше вселяли воістину біблійський страх. І це буде відбуватися не через те, що перестають поступати складні завдання. Тобі (і мені, і всім) стане зрозуміло — "дорогу здолає той, хто йде", "шлях в тисячу лі починається одного кроку".`,
    ...
    howto: {
      ...
      rules: [
        'Як тільки у вас з\'являється завдання (отримуєте email, дзвінок, повідомлення чи просто бачите щось цікаве, що необхідно додати до вашого списку завдань) — одразу ж створюйте завдання. <b>Не чекайте</b>, ви <b>забудете</b> про це завдання, якщо його не запишите. Користуйтеся Telegram-ботом <a target="_blank" href="https://t.me/todorant_bot">@todorant_bot</a> для швидкого створення завдань.',
        ...
        'Не соромтеся приєднуватися до нашої <a target="_blank" href="https://t.me/todorant_feedback">групи</a> у Telegram для обміну досвідом про те, як ставати продуктивніше! Ваші відгуки про роботу сервісу, пропозиції і скарги вкрай вітаються.',
